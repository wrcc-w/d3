...
$(document).ready(function () {
  ...
  $(window).on('load', function () {
    ...
    loadYearlyIncomeChat(start.format('YYYY-MM-D'), end.format('YYYY-MM-D'));
    loadInvoiceViewStatus();
  });

  window.loadPaymentViewStatus = function () {
    $.ajax({
      type: 'GET',
  ...
  window.preparePaymentViewStatusChart = function (result) {
  ...
  window.loadInvoiceViewStatus = function () {
    $.ajax({
      type: 'GET',
  ...
  window.prepareInvoiceViewStatusChart = function (result) {
  ...
  window.loadYearlyIncomeChat = function (startDate, endDate) {
    $.ajax({
      type: 'GET',
  ...
  window.prepareYearlyIncomeViewChart = function (result) {
      ...
      options: {
        ...
        plugins: {
          ...
          tooltip: {
            callbacks: {
              label: function label(context) {
                return ' ' + currency + ' ' + number_format(context.formattedValue);
              }
            }
          }
        },
