<a href="{{route('subscription-plans.show',$row->id)}}" title="Show"
...
<a href="{{route('subscription-plans.edit', $row->id)}}" title="<?php echo __('messages.common.edit') ?>"
   class="btn btn-icon btn-bg-light edit-btn btn-active-color-primary btn-sm me-1">
...
@if(!$row->is_default)
    <a href="#" title="<?php echo __('messages.common.delete') ?>" data-id="{{$row->id}}"
       class="delete-btn btn btn-icon btn-bg-light text-hover-primary btn-sm">
            <span class="svg-icon svg-icon-3">
            <svg xmlns="http://www.w3.org/2000/svg" width="24px" height="24px" viewBox="0 0 24 24" version="1.1">
